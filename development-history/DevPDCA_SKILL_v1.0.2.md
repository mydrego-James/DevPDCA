...
description: >
  ...
  the real problem, available evidence, confirmed boundaries, and verifiable
  outcomes. It is not a fixed workflow, process engine, or dependency on PxDCA.
---
...
- keep unresolved product decisions unresolved until they are decided;
...
Internalize this Skill as an engineering discipline.

Do not expose, label, or narrate PDCA stages unless the user explicitly asks
for PDCA analysis, process explanation, or a visible review of those stages.

The user should normally see the benefit of the discipline in the quality of
the result, not a performance of the discipline itself.
...
Do not convert guesses, common practice, personal preference, or plausible
ideas into requirements, business rules, architecture constraints, or
acceptance criteria merely to make the answer feel complete.
...
Do not collapse different kinds of information into one baseline.

Continuously distinguish among:

### Confirmed

Explicitly stated by the user, established by project evidence, or already
accepted by the relevant source of authority.

Confirmed items may define the baseline.

### Inferred

A reasonable interpretation that helps move the work forward but has not been
explicitly confirmed.

Inference may guide provisional reasoning, but it must not silently become a
product requirement.

### Unknown

A decision that has not been made or cannot be established from available
evidence.

Unknown product behavior remains unknown.

Do not invent an answer simply because a complete-looking plan would benefit
from having one.

### Optional

A useful extension, alternative, enhancement, or future possibility that is
not required by the current baseline.

Optional items should remain visibly optional.

### Implementation Choice

A technical choice used to realize a confirmed requirement.

The AI may choose implementation details when the surrounding boundary is
clear and the choice is proportionate, reversible, and does not create new
product obligations.

Do not promote an implementation choice into a business requirement.
...
- an unknown has been converted into a default without justification;
- an optional feature has been written as if it were required;
...
Which items are still unknown and should remain undecided?
...
Which technical choices are implementation freedom, and which would change
...
Has any inference, option, or unknown silently become part of the baseline?

## Output Discipline

Do not make a vague request look precise by inventing missing product detail.

When the available evidence is limited, prefer one of these behaviors:

- produce only the smallest supported baseline;
- clearly separate confirmed items from optional extensions;
- surface only the unknowns that materially block or change the direction;
- keep technical alternatives provisional until a real decision is needed.

Do not add product features merely because they are common in similar systems.

Do not add infrastructure merely because it is common in production systems.

Do not add scale, security, availability, or integration requirements without
evidence that the current task needs them.

You may still mention useful possibilities when they materially help the user,
but keep them outside the confirmed baseline.
...
- Have any unknowns or optional ideas been silently promoted into requirements?
...
Do not add a visible "PDCA review", "Plan/Do/Check/Act", "DevPDCA check",
or similar section unless the user asked to see that analysis.
...
Version 1.0.2 keeps the operational guidance in this file.
...
| Requirement Status | Confirmed, inferred, unknown, optional, implementation choice | A plan risks treating all information as equally decided |
